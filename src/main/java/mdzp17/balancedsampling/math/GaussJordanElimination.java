...
public class GaussJordanElimination {
    ...
    /**
     * Implementation of the forward elimination
     * finds the maximum of each row,
     * does permutations to put it on top
     * then substract this line with the next ones (using simple division)
     * More info on :
     **/
    private void solve() {
        ...
        if (isFeasible()) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (Math.abs(sum - b[i]) > EPSILON) {
                    System.out.println("error, gaussian solving is not feasible");
                    return false;
                }
            }
            return true;
